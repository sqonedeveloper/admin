var page_actions = function () {
   ...
   $("html").on("click", function () {
      if (html_click_avail)
         $(
            ".x-navigation-horizontal li,.x-navigation-minimized li,.x-features .x-features-search,.x-features .x-features-profile"
         ).removeClass("active");
   });
   ...
   $(".lockscreen-box .user_signin").on("click", function () {
      ...
      $(".user")
         .hide()
         .find("img")
         .attr("src", "assets/images/users/no-image.jpg");
      $(".user").show();
      return false;
   });
   /* END LOCK SCREEN */
...
function page_content_onresize() {
   ...
   if (vpW > 1024) {
      ...
      if ($(".content-frame-body").height() < vpH - 162) {
         ...
         $(
            ".content-frame-body,.content-frame-right,.content-frame-left"
         ).height(cfM);
      } else {
...
function panel_fullscreen(panel) {
   if (panel.hasClass("panel-fullscreened")) {
      ...
      panel
         .find(".panel-body,.chart-holder")
         .height($(window).height() - hplus);
...
function pageLoadingFrame(action, ver) {
   ...
   if (ver === "v2") pl_frame.addClass("v2");

   var loader = new Array();
   loader["v1"] =
      '<div class="page-loading-loader"><img src="img/loaders/page-loader.gif"/></div>';
   loader["v2"] =
      '<div class="page-loading-loader"><div class="dot1"></div><div class="dot2"></div></div>';
